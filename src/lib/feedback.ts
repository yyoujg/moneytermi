...
// 첫 진동(at=0)은 setTimeout에 태우지 않고 즉시 — 타이머는 다음 매크로태스크라 렌더·이벤트 처리 뒤로 밀린다.
const hapticSeq = (steps: [Haptic, number][]) => steps.forEach(([t, at]) => at === 0 ? haptic(t) : setTimeout(() => haptic(t), at));
