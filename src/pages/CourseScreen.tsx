  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 pb-24 overflow-y-auto [&::-webkit-scrollbar]:hidden">
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-gray-100">
        <div className="pt-12 px-5 pb-3">
          ...
          {showSearch && (
            <div className="mt-3 relative">
              ...
              />
              {showResults && (
                <div className="absolute top-[52px] left-0 right-0 bg-white rounded-2xl border border-gray-100 shadow-lg z-50 overflow-hidden">
        ...
        {/* 카테고리 필터 */}
        <div className="px-5 pb-3 overflow-x-auto [&::-webkit-scrollbar]:hidden">
          <SegmentedControl
            value={activeCategory}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setActiveCategory(e.target.value)}
          >
            {allCategories.map(cat => (
              <SegmentedControl.Item key={cat} value={cat} size="small">
                {cat}
          ...
          return (
            <div key={course.id} className="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100">
              ...
              <div
                ...
                className={`px-5 pt-5 pb-4 relative overflow-hidden cursor-pointer active:opacity-90
                  ${isCompleted
              ...
              >
                <div className={`absolute -right-6 -top-6 w-24 h-24 rounded-full opacity-10 ${isCompleted ? 'bg-white' : 'bg-orange-300'}`} />
                <div className={`absolute -right-2 top-6 w-14 h-14 rounded-full opacity-10 ${isCompleted ? 'bg-white' : 'bg-orange-200'}`} />
                ...
                <h3 className={`text-[15px] font-bold leading-tight mb-1 relative ${isCompleted ? 'text-white' : 'text-gray-900'}`}>
                  {course.title}
                </h3>
