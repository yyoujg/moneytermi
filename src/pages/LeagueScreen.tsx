      ...
      <div className="bg-white pt-4 px-5 pb-5">
        ...
        <div className="bg-[#F7F7F7] rounded-2xl p-4 mb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-right">
              <p className="text-xs text-[#AAAAAA] mb-0.5">현재 순위</p>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {above2User && (
              <div className="rounded-xl px-3 py-2.5 flex items-center justify-between bg-[#F0F0F0]">
                <div className="flex items-center gap-2">
                  <TrendingUp size={13} className="text-[#AAAAAA] shrink-0" />
                  <span className="text-xs text-[#AAAAAA] truncate max-w-[120px]">{above2User.name}</span>
                </div>
                <span className="text-sm font-bold text-[#888888]">+{pointsToAbove2}P → {myRank - 2}위</span>
              </div>
            )}
        ...
        <div className="relative">
          <div className="flex justify-between items-start relative">
            <div className="absolute top-4 left-4 right-4 h-[2px] bg-[#E5E5E5] z-0 rounded-full">
              <div
              ...
              return (
                <div key={tier.id} className="flex flex-col items-center relative z-10 w-14">
                  ...
                  <span className={`text-[10px] font-medium text-center mt-1.5 leading-tight
                    ${isCurrent ? 'text-orange-500' : isPast ? 'text-[#AAAAAA]' : 'text-[#C0C0C0]'}`}>
                    {tier.name}
                  </span>
                </div>
              );
      ...
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden px-5 pt-4 pb-24">
        ...
        <div className="bg-white rounded-2xl overflow-hidden mb-3">
          <div className="px-4 pt-4 pb-3">
            <p className="text-[11px] font-medium text-[#AAAAAA] mb-4 text-center tracking-widest uppercase">Top 3</p>
            <div className="flex items-end justify-center gap-3 mb-2">
              {top3[1] && (
                <div className="flex flex-col items-center flex-1">
                  <div className="w-11 h-11 bg-[#F0F0F0] rounded-full flex items-center justify-center text-xl mb-1">{top3[1].emoji}</div>
                  <div className="w-full bg-[#F0F0F0] rounded-t-xl flex flex-col items-center py-3" style={{ height: 68 }}>
                    <span className="text-base mb-0.5">🥈</span>
                    <p className="text-[10px] font-bold text-[#555555] truncate w-full text-center px-1">{top3[1].name}</p>
                    <p className="text-[10px] font-medium text-[#AAAAAA]">{top3[1].points}P</p>
                  </div>
                </div>
              )}
              {top3[0] && (
              ...
              {top3[2] && (
                <div className="flex flex-col items-center flex-1">
                  <div className="w-11 h-11 bg-[#F0F0F0] rounded-full flex items-center justify-center text-xl mb-1">{top3[2].emoji}</div>
                  <div className="w-full bg-[#F0F0F0] rounded-t-xl flex flex-col items-center py-3" style={{ height: 56 }}>
                    <span className="text-base mb-0.5">🥉</span>
                    <p className="text-[10px] font-bold text-[#555555] truncate w-full text-center px-1">{top3[2].name}</p>
                    <p className="text-[10px] font-medium text-[#AAAAAA]">{top3[2].points}P</p>
                  </div>
                </div>
              )}
            </div>
                ...
                <ListRow
                  ...
                  left={
                    <div className="w-8 text-center shrink-0">
                      {rank <= 3
                        ...
                        : <span className={`text-sm font-bold ${isMe ? 'text-orange-500' : 'text-[#AAAAAA]'}`}>{rank}</span>
                      }
                    </div>
                  }
                  contents={
                    <div className="flex items-center gap-2 min-w-0">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center text-lg shrink-0 ${isMe ? 'bg-orange-500/15' : 'bg-[#F0F0F0]'}`}>
                        {u.emoji}
                      </div>
                      <div className="min-w-0">
                        <p className={`text-sm font-semibold truncate ${isMe ? 'text-orange-500' : 'text-[#111111]'}`}>
                          {u.name}
                  ...
                  right={
                    <div className="flex items-center gap-1 shrink-0">
                      <Zap size={12} className={`fill-current ${isMe ? 'text-orange-500' : 'text-[#AAAAAA]'}`} />
                      <span className={`text-sm font-bold ${isMe ? 'text-orange-500' : 'text-[#555555]'}`}>{u.points}</span>
                    </div>
                  }
                />
              );
        ...
        <div className="flex gap-2 mb-4">
          <div className="flex-1 flex items-center gap-2 bg-white rounded-xl px-3 py-2.5">
            ...
            <span className="text-[11px] text-[#AAAAAA]">상위 {promoteZone}명 승급</span>
          </div>
          <div className="flex-1 flex items-center gap-2 bg-white rounded-xl px-3 py-2.5">
            <div className="w-2 h-2 rounded-full bg-red-400 shrink-0" />
            <span className="text-[11px] text-[#AAAAAA]">하위 {demoteZone}명 강등</span>
          </div>
        </div>
