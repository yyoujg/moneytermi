...
const GuestLinkSheet = ({ onClose, onLink }: Props) => {
  ...
  const handleVerify = async () => {
    ...
    if (stored?.guestToken) {
      try {
        ...
        console.warn('link_guest_to_auth 실패:', e);
      }
    }
  ...
  return (
    <>
      ...
      >
        <div className="bg-[#161616] rounded-t-3xl px-5 pt-6 pb-10 flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {step === 'otp' && (
                <button
                  ...
                  className="w-7 h-7 flex items-center justify-center rounded-full bg-[#1E1E1E] mr-1"
                >
                  <ArrowLeft size={14} className="text-[#777777]" />
                </button>
              )}
            ...
            >
              <X size={15} className="text-[#777777]" />
            </button>
          </div>

          {/* 설명 카드 */}
          <div className="bg-[#1E1E1E] rounded-2xl p-4 flex gap-3">
            <Shield size={18} className="text-orange-400 shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold text-white mb-0.5">지금까지의 기록을 지키세요</p>
          ...
          {step === 'email' && (
            <div className="flex flex-col gap-3">
              <div>
                <p className="text-xs text-[#555555] mb-2">이메일 주소</p>
                <input
                  type="email"
                  value={email}
                  ...
                  className="w-full px-4 py-3.5 rounded-xl text-sm text-white placeholder-[#555555] outline-none"
              ...
              <button onClick={onClose} className="text-xs text-[#555555] text-center py-1 active:opacity-70">
                나중에 하기
              </button>
            </div>
          )}
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3.5 rounded-xl text-sm text-white placeholder-[#555555] outline-none text-center ${otpError ? 'ring-1 ring-red-500/60' : ''}`}
                  style={{ backgroundColor: '#1E1E1E', letterSpacing: otp ? '0.3em' : undefined }}
                  autoFocus
                />
                {otpError && (
              ...
              <button
                onClick={handleSendCode}
                disabled={loading}
                className="text-xs text-[#555555] text-center py-1 active:opacity-70 disabled:opacity-30"
              >
                코드 재발송
              </button>
            </div>
