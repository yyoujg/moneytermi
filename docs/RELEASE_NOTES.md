...
|------|--------------------|--------------------|-----------|
| 2026092x-1xx (콘솔 확인 후 기입, deploymentId `01a0dd2e`) | 배포됨 2026-09-26 19:06 KST · 검수 요청 전 (PR #51) | — | 포인트·XP 점검 — 미션 진행도·잔고 즉시 갱신, 복습 획득 포인트 서버값, 부스트 연타 가드 (서버 보강 SQL은 적용 완료) |
| 2026092x-1xx (콘솔 확인 후 기입, deploymentId `01a0dcd6`) | 배포됨 2026-09-26 17:30 KST · 검수 요청 전 (PR #50) | — | 단어카드 '자세히' 본문 용어 하이라이트, 뉴스 키워드 정규식 이스케이프 |
| 20260926-139 (deploymentId `01a0dbf2`) | 2026-09-26 16:58 (생성 13:21, PR #49) | 2026-09-26 | **단어 뜻 747개 전면 재작성**·슬래시 항목 분리·퀴즈 정답 노출 수정, 닉네임 저장 버그, 포인트 축하 모달, 진동 반응 속도 |
| 20260923-129 (콘솔 확인 후 기입) | 출시 대기 | — | 데일리 푸시 발송 코드 PUSH2 → PUSH3 (알림 클릭 시 딥링크 동작) |
| 20260925-138 (deploymentId `01a0d5f9`) | 2026-09-25 10:09 (생성 09:31, PR #41~#48) | 2026-09-25 | 앱 전면 개편: 학습 코스 → 세로 패스(주제별 37코스, 코스 간 잠금), 리그(주간 XP 랭킹), XP/포인트 분리 + **포인트 경제(레슨 10P · XP 50마다 +50P · 부족 시 광고)**, 미션 5종·8시간 리셋, 배지, 랜덤 닉네임, 전역 상단바, 800선 716단어 교체, 전역 모션 |
...
| `20260925-138` | 09-25 09:31 | 09-25 10:09 | 38분 |
| `20260926-139` | 09-26 13:21 | 09-26 16:58 | 3시간 37분 |
...
## 2026-09-26 머지 / 19:06 배포 · 검수 요청 전 (PR #51) — 포인트·XP 시스템 점검

CI run 36234770094 (10:06 UTC, 40초). **검수용 deploymentId `01a0dd2e-750a-7368-80c6-1ad64982a935`**,
실기기 테스트 링크 `intoss-private://moneytermi?_deploymentId=01a0dd2e-750a-7368-80c6-1ad64982a935&host=appsInTossHost`.

코드 기준 점검(서버 SQL은 위 적용 표의 버전을 기준). 서버 보강 SQL 4개(1·2a·2b·3)는 **적용 완료**(확인 쿼리 전항목 true). 클라이언트 수정은 PR #51 머지·배포 후 반영.
...
실기기 테스트 링크 `intoss-private://moneytermi?_deploymentId=01a0dcd6-93f2-706a-8546-9b9bb4f0f5d8&host=appsInTossHost`. 서버 변경 없음(코드만).

### 🖍 '자세히' 본문 용어 하이라이트 (`pages/WordCardScreen.tsx`, `lib/quiz.ts`)

- 단어카드 '자세히' 문단에서 그 용어가 나올 때마다 뉴스 키워드와 같은 주황 형광(`<mark>`, `--color-brand-cream`)으로 표시. 기본형·슬래시 조각·괄호 약어(`국내총생산`, `GDP`)까지 잡고 한글은 띄어쓰기가 달라도(`국내 총생산`) 잡는다.
- 용어 패턴 생성을 `termPattern`으로 분리해 퀴즈의 정답 가림(`maskTerm`)과 같은 범위를 본다.
- 뉴스 `Highlight`가 키워드를 이스케이프 없이 정규식에 넣어 괄호 든 단어에서 예외가 날 수 있던 것 수정.
- 검토 시 숫자·관련 용어 굵게는 선택하지 않아 넣지 않았다. 필요하면 같은 `Highlight`에 규칙만 추가.

### 검증

- vitest 111개(`termPattern` 케이스 추가)·`tsc` 통과. `WordCardScreen.tsx`의 기존 eslint 오류 1건(Math.random 순수성)은 무관.
- 실기기에서 '자세히' 단계의 형광이 과하지 않은지 확인 필요.

### 토스 콘솔 출시노트 (사용자 노출용)

```
자세히 읽기가 편해졌어요
- 단어 카드 '자세히'에서 지금 배우는 용어가 나올 때마다 형광으로 표시돼 어디를 봐야 할지 한눈에 들어와요
- 괄호나 약어가 붙은 용어도 빠짐없이 표시돼요
```

---

## 2026-09-26 머지 / 2026-09-26 16:58 출시 (번들 20260926-139, PR #49) — 단어 뜻 전면 재작성 · 퀴즈 정답 노출 수정

CI run 36217579714 (04:21 UTC, 38초). **검수용 deploymentId `01a0dbf2-9826-7426-8e8d-f60cf4ad8948`**,
실기기 테스트 링크 `intoss-private://moneytermi?_deploymentId=01a0dbf2-9826-7426-8e8d-f60cf4ad8948&host=appsInTossHost`.
서버 쪽 단어 SQL(`words_bok/16_*`, `17_courses_*`)은 **머지 전에 적용·검증 완료** — 이 번들 전후로 추가 SQL 없음.

### 🔎 발단 — "단어 뜻·퀴즈 잘못된 게 너무 많다"

라이브 `words` 716행을 덤프해 점검한 결과:

| 문제 | 규모 |
|---|---|
| 뜻(`meaning`)이 한국은행 본문 첫 문장을 110자에서 자른 것. 문장 중간에서 끊김 | 189행 |
| 첫 문장이 정의가 아니라 배경 설명(기회비용 → "…희소성이라고 한다") → "이 뜻에 맞는 용어는?" 문제 자체가 틀림 | 약 50행 |
| 복습 초성 힌트가 괄호·슬래시까지 포함(`ㄱㅅㅈㅅㄱㄱ()`) | 181행 |
| 본문에 PDF 줄이음 자국(`의 미한다`, `이론이 다`, ` ABC `) | 138행 |
| 관련 용어 앞에 제어문자 → 렌더에서 매칭 실패 | 4행 |
| 퀴즈 `maskTerm`이 괄호 약어(GDP)·슬래시 뒷조각·띄어쓰기 변형을 못 가려 문제에 정답 노출 | 73행 |
| '단리/복리'처럼 두 개념이 한 단어 | 47행 |

### 📚 데이터 — 뜻 747개 전면 재작성 (`supabase/words_bok/meanings.py` + `refine.py`)

- 뜻 전부를 **초보자용 한 줄**(명사구, 45자 이내, 용어 자체 미포함)로 새로 썼다. 예: 기회비용 → "하나를 선택하면서 포기한 다른 선택지의 가치". 원문(한국은행)은 '자세히' 본문에 그대로 남아 있다.
- 슬래시 항목 28개를 개념별로 **분리해 신규 31행**(id 717~747). 부모 진도를 자식에 복사해 코스 재잠금 방지. 동의어 묶음 17개는 이름만 정리(`환매조건부매매(RP)`, `그림자금융(NBFI)`). 총 **747단어**.
- 힌트를 기본형 초성으로(영문은 머리글자: `Treasury Bill` → `TB`). 관련 용어 제어문자 제거·분리 이름 치환. 본문 줄이음 자국은 말뭉치 빈도 기반으로 89건 결합 + ` ABC ` 제거.
- 코스는 `categories.py`로 17주제 **39개** 재생성(37 → 39). 진도(`word_progress`) 보존.
- 적용 경위: SQL Editor가 45줄 넘는 붙여넣기의 앞부분만 실행해 3차례 부분 적용 → `*_rest.sql`로 보충. 최종 덤프 검증 words 747·courses 39·course_words 747·위반 0, `trg_word_progress_mission` 활성 확인. `parse800.py`는 은퇴(재실행 시 손으로 쓴 뜻을 덮어씀).

### 🧩 퀴즈 · 복습 (`lib/quiz.ts`)

- `maskTerm`이 전체 단어·기본형뿐 아니라 슬래시 각 조각·괄호 약어를 가리고, 한글은 띄어쓰기가 달라도 가린다. 새 뜻에는 용어가 없어 주로 복습 상세 본문에서 동작.
- 오답 보기에서 정답과 기본형 앞 3글자가 같은 용어(통화정책수단 vs 통화정책체계) 제외. 같은 주제 우선 규칙은 유지.

### 🐛 닉네임 변경이 다른 사람 리그에 반영되지 않던 것 (`hooks/useAuth.tsx`)

- 기본 `supabase` 클라이언트로 `profiles`를 UPDATE → `profiles_update` RLS가 `x-guest-token` 헤더로 본인 행을 찾으므로 0행 갱신, PostgREST는 에러를 주지 않음 → 로컬 상태·저장소만 바뀌고 서버는 옛 닉네임. 게스트 토큰 클라이언트로 요청하고 `.select('id')`로 갱신 행을 확인, 0행이면 실패 처리.
- 이미 로컬에만 바뀐 사용자는 닉네임을 한 번 더 저장해야 서버에 반영된다.

### 🎉 포인트 획득 축하 모달 (`components/PointCelebration.tsx`)

- 미션 보상 수령(+P·+5 XP)과 광고 보상(+P)의 토스트를 색종이 모달로 교체. 실패 안내·부스트 구매 토스트는 그대로.

### 📳 진동 반응 속도 (`lib/feedback.ts`, `hooks/useTapHaptics.ts`)

- 첫 진동이 `setTimeout(…, 0)`에 실려 다음 매크로태스크로 밀리던 것 → 즉시 호출. 11개 피드백에서 효과음 준비 뒤에 진동을 보내던 순서를 진동 → 소리로. 탭 진동을 효과음 프리로드보다 먼저.
- 시퀀스 간격·강도는 그대로.

### 검증

- vitest 110개(퀴즈 테스트 4개 추가)·`tsc -p tsconfig.app.json`·eslint(변경 파일) 통과. 라이브 DB 덤프 대조 스크립트로 최종 확인.
- 브라우저 프리뷰는 토스 앱 밖 게스트 인증(401)으로 퀴즈·닉네임·모달을 직접 확인하지 못함 → 실기기 테스트 링크로 확인 필요.

### 토스 콘솔 출시노트 (사용자 노출용)

```
단어 뜻이 훨씬 쉬워졌어요
- 747개 경제·금융 용어의 뜻을 초보자도 바로 이해할 수 있는 한 줄로 전부 다시 썼어요
- '단리/복리'처럼 한데 묶여 있던 용어를 각각의 단어로 나눠 하나씩 배울 수 있어요
- 퀴즈 문제에 정답이 비치거나 헷갈리는 비슷한 이름의 보기가 나오던 문제를 고쳤어요
- 복습 힌트가 용어의 초성만 정확히 보여줘요
- 닉네임을 바꾸면 리그에서도 바로 바뀌어요
- 미션·광고 보상을 받으면 축하 화면이 떠요
- 진동이 더 빠르게 반응해요
```

### 미해결

- 분리된 자식 단어의 '자세히' 본문은 부모 원문 그대로라 두 개념을 함께 설명한다.
- 본문 줄이음 자국 중 `간접금융에 서는`처럼 두 글자 이상 조각은 남아 있다(말뭉치 빈도로 안 잡힘).
- `useAuth.tsx`의 기존 eslint 오류 3건(fast-refresh·선언 전 접근)은 이번 변경과 무관해 그대로.

---

## 2026-09-25 머지 / 2026-09-25 10:09 출시 (번들 20260925-138, PR #41~#48) — 앱 전면 개편
...
| `migration_economy_hardening_1, 2a, 2b, 3.sql` | ✅ 적용 완료 (2026-09-26, PR #51) — 확인 쿼리 7항목 모두 true(add_xp·quiz·mission 함수, quiz_rewards_daily, word_progress FK, profiles_insert 정책, KST 상한) — **포인트·XP 보강**. ① `add_xp` 한 문장 갱신(경합 시 XP 유실·50P 이중 지급 방지) ② `claim_mission_reward` 조건부 UPDATE로 이중 수령 차단 + XP 지급 후 잔고 반환 ③ 퀴즈 보상 하루 150정답 상한(`quiz_rewards_daily`)·typed 콤보 보너스 +18 상한 ④ `word_progress.word_id` FK ⑤ `profiles_insert` 정책의 points=0 조건 제거(기본값 100과 충돌) ⑥ 광고·추천 일일 상한 KST. 순서 1→2a→2b→3. Supabase 에디터의 "새 테이블 RLS 자동 추가"가 CREATE TABLE과 같은 붙여넣기의 `$$` 함수를 `;`로 쪼개므로 테이블(2a)과 함수(2b)를 분리. **옛 마이그레이션 재실행 금지**(함수가 옛 버전으로 되돌아감) |
