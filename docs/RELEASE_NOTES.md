...
| 20261000-1xx (10월, 미머지) | 출시 대기 | — | 앱 전면 개편: 학습 코스 → 세로 패스, 리그(주간 XP 랭킹), XP/포인트 분리, 미션 5종·8시간 리셋, 배지, 랜덤 닉네임, 전역 상단바 |
...
## 2026-09-24 커밋 / 출시 대기 (10월 번들, PR #41) — 앱 전면 개편

9/30 전수 점검이 끝난 뒤 별도 번들로 출시하기로 한 묶음. 화면 구조·보상 체계·
데이터 모델이 함께 바뀌어 항목이 많다. **서버 마이그레이션 5개가 선행돼야 한다**(아래 표).
...
- **전역 상단바** 신설 — `머니터미 🔥N ✨NXP ⚡NP 📖N`. 포인트를 누르면 상점 시트
  (광고 충전 / XP 2배 부스트).
...
| `migration_xp.sql` | ❌ **미적용** — 적용 전까지 XP가 0으로 고정되고 리그가 빈 채로 보인다 |

`point_events`/`xp_events`는 적용 시점부터 이력이 쌓이므로 **첫 주 랭킹은 모두 0에서 시작**한다.

### 미해결

- 경제금융용어 800선 PDF 교체 — 추출한 본문이 다른 용어와 뒤섞이는 사례를 확인해 폐기.
  방향 재결정 필요.
