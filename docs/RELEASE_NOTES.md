...
|------|--------------------|--------------------|-----------|
| 20260923-126 (콘솔 확인 후 기입) | 검수 대기 (124·125 반려 후 재등록) | — | PR #33·#34·#36·#37·#38·#39 통합: 계측 보정, 뉴스 링크 Device.openURL 전환, CI env 주입(리워드 광고·친구 초대 실제 도달), NavBar 가림 해소, 자체 뒤로가기 4곳 제거, MY→마이 |
...
## 2026-09-22 커밋 / 검수 대기 (번들 126, PR #34·#36·#37) — 하단 NavBar 가림 해소 + 자체 뒤로가기 제거 + 검수 문구 대응

아래 PR #33 항목과 **같은 번들**로 출시. 9/22 CI 배포 4회(13:57 #33 → 14:46 #34 → 14:52 #36 → 15:10 #37,
커밋 `0b41b6d`) 중 **15:10 번들만 검수 요청**한다(앞 3개는 콘솔에서 미출시로 둔다). 콘솔 출시노트는 PR #33 항목 참고.
...
## 2026-09-22 커밋 / 검수 대기 (번들 126, PR #33) — 계측 보정 + 뉴스 외부 링크 제거 + CI env 주입
...
✅ **실기기 확인 완료(9/23, 번들 126 `4a940e9`)**: 토스 앱에서 뉴스 항목 탭 → 원문이 외부 브라우저로 열림.
`Device.openURL` 전환이 반려 사유를 해소한다는 것이 실측으로 확정됐다(추론 단계 종료). 번들 126 재등록 대상.
