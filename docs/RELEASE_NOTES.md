...
| `migration_economy_hardening_1, 2a, 2b, 3.sql` | ⏳ 1·3 적용, 2a·2b 대기 (2026-09-26, PR #51) — **포인트·XP 보강**. ① `add_xp` 한 문장 갱신(경합 시 XP 유실·50P 이중 지급 방지) ② `claim_mission_reward` 조건부 UPDATE로 이중 수령 차단 + XP 지급 후 잔고 반환 ③ 퀴즈 보상 하루 150정답 상한(`quiz_rewards_daily`)·typed 콤보 보너스 +18 상한 ④ `word_progress.word_id` FK ⑤ `profiles_insert` 정책의 points=0 조건 제거(기본값 100과 충돌) ⑥ 광고·추천 일일 상한 KST. 순서 1→2a→2b→3. Supabase 에디터의 "새 테이블 RLS 자동 추가"가 CREATE TABLE과 같은 붙여넣기의 `$$` 함수를 `;`로 쪼개므로 테이블(2a)과 함수(2b)를 분리. **옛 마이그레이션 재실행 금지**(함수가 옛 버전으로 되돌아감) |
