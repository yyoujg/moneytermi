...
| 2026092x-1xx (콘솔 확인 후 기입, deploymentId `01a0d5f9`) | 배포됨 09:34 KST · 검수 요청 전 (2026-09-25 머지, PR #41~#48) | — | 앱 전면 개편: 학습 코스 → 세로 패스(주제별 37코스, 코스 간 잠금), 리그(주간 XP 랭킹), XP/포인트 분리 + **포인트 경제(레슨 10P · XP 50마다 +50P · 부족 시 광고)**, 미션 5종·8시간 리셋, 배지, 랜덤 닉네임, 전역 상단바, 800선 716단어 교체, 전역 모션 |
...
## 2026-09-25 머지 / 출시 대기 (PR #41~#45, 검수 요청 전) — 앱 전면 개편

당초 9/30 전수 점검 뒤 머지하기로 했으나 9/25에 머지·배포로 앞당김(콘솔 검수 요청 시점은 별도 판단).
CI 배포 9회(중복 1 포함): run 36031409503(02:02, `01a0d45d`) → 상단 여백 → PR #42(08:03, `01a0d5a9`) → 실기기 깜박임·멈춤 → PR #43(08:32, `01a0d5c2`) → 효과음·진동 → PR #44(08:38, `01a0d5c8`) → 배지 축하 → PR #45 run 36074144743(08:44, `01a0d5cd`) → 단어카드 4단계·퀴즈 의미 패널 → PR #46 run 36075572069(09:01, `01a0d5de`; 직후 docs 푸시로 동일 코드 `01a0d5df` 중복 빌드) → 진동 강화·버튼 효과음 → PR #47 run 36076032221(09:12, `01a0d5e3`) → 진동 최대·버그 8건·효과음 파일 → PR #48 run 36077915839(09:34).
**검수용 최신 deploymentId `01a0d5f9-8a1a-7636-86a2-c51f09010418`**,
실기기 테스트 링크 `intoss-private://moneytermi?_deploymentId=01a0d5f9-8a1a-7636-86a2-c51f09010418&host=appsInTossHost`.

화면 구조·보상 체계·데이터 모델이 함께 바뀌어 항목이 많다. 서버 마이그레이션 7개 + 단어 SQL 20개는 **전부 적용 완료**(아래 표) — 이 번들이 나가도 추가 SQL은 없다.
...
### 2026-09-26 콘텐츠 점검 — 단어 뜻 · 퀴즈

라이브 `words` 716행을 덤프해 점검한 결과: 뜻이 원문 첫 문장을 110자에서 자른 것이라 189행이 문장 중간에서 끊기고 약 50행은 정의가 아닌 배경 설명(기회비용 → "…희소성이라고 한다")이라 퀴즈 문제 자체가 틀렸다. 힌트 181행이 괄호·슬래시까지 초성 처리(`ㄱㅅㅈㅅㄱㄱ()`), 본문에 PDF 줄이음 자국(`의 미한다`, ` ABC `), related_words 4행에 제어문자.

- **데이터**(`words_bok/meanings.py` + `refine.py`, SQL 적용 대기): 뜻 747개 전부 초보자용 한 줄(명사구, 45자 이내, 용어 미포함)로 재작성. '단리/복리'처럼 두 개념이 한 행이던 28개를 분리(신규 31행, 부모 진도 복사로 코스 재잠금 방지), 동의어 묶음 17개는 이름만 정리(`환매조건부매매(RP)`). 힌트는 기본형 초성(영문은 머리글자). 코스 17주제 39개로 재생성.
- **퀴즈**(`lib/quiz.ts`): `maskTerm`이 괄호 약어(GDP)·슬래시 뒷조각·띄어쓰기 다른 형태를 못 가려 70+3행에서 정답이 노출되던 것 수정. 오답 보기에서 정답과 기본형 앞 3글자가 같은 용어(통화정책수단 vs 통화정책체계) 제외.
- `parse800.py` 은퇴 — 재실행하면 손으로 쓴 뜻을 덮어쓴다.

- **배지 획득 축하**(9/25): 마이페이지 진입 시 새로 얻은 배지를 색종이 오버레이로 축하(여러 개면 한 화면에), 전용 효과음(상승 아르페지오)+축포 햅틱. '이미 보여준 배지'는 기기에 기록. 라우트 페이드의 fill-mode가 스택 컨텍스트를 남겨 축하 오버레이 위로 NavBar/TopBar가 보이던 것 수정.
...
- **단어카드 4단계 분리**(9/25, PR #46): 한 카드에 몰려 있던 내용을 **뜻 → 자세히 → 뉴스 → 관련 용어** 페이지로 나눴다. 상단 단계 칩, 다음 버튼 라벨은 다음 단계 이름, 내용 없는 단계는 건너뛴다. 자세히 본문(평균 500자)은 문장 2개씩 문단으로 나눠 표시.
- **'좋아요!' 패널을 퀴즈로 이동**(9/25, PR #46): 단어 완료 시 뜨던 하단 패널을 없애고(완료 시 효과음·햅틱만), 퀴즈 응답 직후에 띄운다 — 정답이면 초록(좋아요!·+NP·콤보·의미), 오답이면 빨강(아쉬워요·정답·의미). 0.3초 자동 진행을 없애고 '계속하기/결과 보기' 버튼으로 넘어가 뜻을 읽을 시간을 준다.
- **진동 강화 + 버튼 효과음**(9/25, PR #47): 실기기에서 약하다는 피드백으로 모든 햅틱을 한 단계씩 올렸다 — 버튼 틱 tickMedium→basicMedium, 정답 success 2연타(콤보 3부터 축포, 5는 축포 3발), 오답·실패 error 2연타, 완료·보상·부스트·스트릭에 축포 한 발씩 추가. 버튼을 누르면 40ms 낮은 '톡' 소리도 함께 난다(효과음 설정을 따름).
- **진동 최대**(9/25, PR #48): 버튼 틱을 success로, 모든 이벤트를 success/confetti/error 조합만으로. SDK에 세기 조절이 없어 가장 강한 패턴을 겹쳐 쓰는 방식. 실기기에서 과하면 `feedback.ts`의 tick 한 줄만 낮추면 된다.
- **버그 점검 8건 수정**(9/25, PR #48): ① 관련 용어 클릭으로 더 짧은 코스의 단어로 이동하면 이전 index가 남아 범위 밖 접근(크래시) 또는 가짜 '학습 완료' → 단어카드를 navigation마다 재마운트(`key={location.key}`). ② 레슨 노드 연타 시 `spend_points` 이중 차감 → in-flight 가드. ③ 미션 보상 연타 시 RPC 이중 호출 → 미션별 in-flight 가드. ④ 미션 진행도 행이 정의보다 먼저 도착하면 m2/m4/m5 진행도가 0으로 → 행을 보관해 정의 도착 시 재병합. ⑤ 복습 정답 후 0.9초 자동 진행 중 '건너뛰기'를 누르면 한 단어 건너뜀 → 채점 중 비활성. ⑥ 전역 탭 리스너의 `:has()` 선택자가 구형 웹뷰에서 예외 → 제거. ⑦ iOS에서 AudioContext가 suspended인 동안 예약된 소리가 첫 resume 때 한꺼번에 터짐 → running일 때만 예약. ⑧ 설정 저장 Promise 미처리 거부.
- **효과음 파일 지원**(9/25, PR #48): `public/sfx/<이름>.mp3`(18종)가 있으면 파일을, 없으면 합성음을 튼다. 첫 탭에서 프리로드, 오디오가 아닌 응답(404 페이지 등)은 없음으로 처리. 파일 목록·용도는 `public/sfx/README.md`. **아직 파일은 없다** — 뮤팟은 유료 요금제의 '소프트웨어 > 어플' 항목 확인 후 직접 받아 넣을 것(myinstants는 라이선스 불명으로 제외).
...
```
머니터미가 새로워졌어요
- 학습 코스가 한 줄 패스로 바뀌었어요. 다음에 누를 곳이 한눈에 보이고, 앞 코스를 끝내면 다음이 열려요
- 한국은행 경제금융용어 800선으로 단어를 전면 교체하고 17개 주제로 나눴어요
- 매주 초기화되는 XP 리그와 티어(브론즈~다이아)가 생겼어요
- 배지를 새로 얻으면 축하 화면이 뜨고, 7일 연속 학습에는 특별한 축하가 기다려요
- 미션은 8시간마다 새로 열려요. 보상을 받으면 포인트와 XP가 함께 들어와요
- 레슨 시작에 포인트가 들어요. 퀴즈·미션·광고로 모으고, XP 50마다 50P를 드려요
- 단어 카드가 뜻·자세히·뉴스·관련 용어 페이지로 나뉘어 읽기 편해졌어요
- 퀴즈를 풀면 정답과 뜻을 바로 보여드려요
- 버튼을 누를 때마다 진동과 효과음이 나고, 정답·완료 진동은 더 또렷해졌어요 (설정에서 끌 수 있어요)
```
