...
| 20261000-1xx (10월, 미머지) | 출시 대기 | — | 앱 전면 개편: 학습 코스 → 세로 패스, 리그(주간 XP 랭킹), XP/포인트 분리, 미션 5종·8시간 리셋, 배지, 랜덤 닉네임, 전역 상단바, 단어 전면 교체(경제금융용어 800선 716개) |
...
데이터 모델이 함께 바뀌어 항목이 많다. **서버 마이그레이션 6개 + 단어 교체 SQL 12개가 선행돼야 한다**(아래 표).
