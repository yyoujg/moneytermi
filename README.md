...
- **지속 학습 동기부여 부족**: `constants.ts`의 5단계 캐릭터 성장(알개미~슈퍼개미)과 콤보 기반 퀴즈 포인트, 데일리 미션(출석/퀴즈 3회)이 반복 방문을 유도하는 장치로 보인다.
...
- **캐릭터 키우기**: `LeagueScreen`이 `getGrowthStage(points)`로 서버 권위 `points`에서 성장 단계(알개미~슈퍼개미)를 클라이언트에서 계산해 보여준다. 다른 유저와의 순위 비교는 없는 솔로 진행형이다(2026-08-31, 기존 리더보드 방식에서 전환).
