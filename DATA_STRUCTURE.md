...
> ⚠️ **코스 수 표기 불일치(확인 필요)**: `APP_INTRO.md`는 "총 34개 코스(경제 19·금융 15)"로 기술하나, 현재 `courses` 조회 결과는 22행이다(bok_* 12 + 시드 10). bok_* id에 결번(bok_1,3,6,8,12,16,20,23,26,29,32,34)이 있어 과거 통합/삭제 이력 가능성. APP_INTRO 수치 갱신 또는 코스 적재 상태 점검 필요.
