-- ===== 콘텐츠 정제 18/36 (refine.py 생성 — 손으로 고치지 말고 meanings.py를 고친 뒤 다시 실행) =====
...
UPDATE public.words SET word = '원·위안 직거래시장', meaning = '달러를 거치지 않고 원화와 위안화를 바로 바꾸는 외환시장', hint = 'ㅇㅇㅇㅈㄱㄹㅅㅈ', related_words = ARRAY['외환시장']::text[] WHERE id = 421;  -- 원/위안 직거래시장
UPDATE public.words SET word = '원금리스크', meaning = '외환결제에서 내 돈은 이미 보냈는데 상대가 망해 받을 돈을 못 받는 위험', hint = 'ㅇㄱㄹㅅㅋ', related_words = ARRAY['외환결제리스크','Herstatt 리스크']::text[] WHERE id = 422;  -- 원금리스크
UPDATE public.words SET word = '원금이자분리채권(STRIPS)', meaning = '이표채의 원금과 이자를 따로 떼어 각각 무이표채처럼 거래하는 채권', hint = 'ㅇㄱㅇㅈㅂㄹㅊㄱ', related_words = ARRAY['이표채','듀레이션']::text[] WHERE id = 423;  -- 원금이자분리채권(STRIPS)
UPDATE public.words SET word = '위조지폐', meaning = '스캐너·프린터 등으로 진짜처럼 보이게 만든 가짜 지폐', hint = 'ㅇㅈㅈㅍ', related_words = '{}'::text[] WHERE id = 424;  -- 위조지폐
UPDATE public.words SET word = '위험가중자산', meaning = '자산마다 위험도에 따라 가중치를 곱해 합산한 은행 자산(자기자본비율의 분모)', hint = 'ㅇㅎㄱㅈㅈㅅ', related_words = ARRAY['BIS 자기자본비율','익스포저','내부등급법','위험가중치']::text[] WHERE id = 425;  -- 위험가중자산/위험가중치
