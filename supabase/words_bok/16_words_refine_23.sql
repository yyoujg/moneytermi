-- ===== 콘텐츠 정제 23/36 (refine.py 생성 — 손으로 고치지 말고 meanings.py를 고친 뒤 다시 실행) =====
-- 뜻 전면 재작성·슬래시 항목 분리(신규 31행)·힌트 기본형 초성·related_words 정리·본문 줄이음 자국 제거.
-- 적용 순서: 16_words_refine_1..N -> 17_courses.sql

UPDATE public.words SET word = '주가지수옵션', meaning = '주가지수를 정해진 가격에 사거나 팔 권리를 거래하는 파생상품', hint = 'ㅈㄱㅈㅅㅇㅅ', related_words = ARRAY['주가지수선물거래']::text[] WHERE id = 526;  -- 주가지수옵션
UPDATE public.words SET word = '주당순이익(EPS)', meaning = '당기순이익을 총 주식 수로 나눈 주식 한 주당 이익', hint = 'ㅈㄷㅅㅇㅇ', related_words = '{}'::text[] WHERE id = 527;  -- 주당순이익(EPS)
UPDATE public.words SET word = '주식시장', meaning = '주식을 새로 발행하고 사고파는 시장', hint = 'ㅈㅅㅅㅈ', related_words = ARRAY['유통시장','발행시장']::text[] WHERE id = 528;  -- 주식시장
UPDATE public.words SET word = '주주환원정책', meaning = '배당 확대와 자사주 매입으로 주주에게 이익을 돌려주는 정책', hint = 'ㅈㅈㅎㅇㅈㅊ', related_words = '{}'::text[] WHERE id = 529;  -- 주주환원정책
UPDATE public.words SET word = '주택저당증권(MBS)', meaning = '주택담보대출을 기초자산으로 발행하는 자산유동화증권', hint = 'ㅈㅌㅈㄷㅈㄱ', related_words = ARRAY['자산유동화','모기지대출']::text[] WHERE id = 530;  -- 주택저당증권(MBS)
