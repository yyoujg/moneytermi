...
CREATE TABLE IF NOT EXISTS public.quiz_rewards_daily ( user_id UUID    NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
date    DATE    NOT NULL, count   INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (user_id, date) );
ALTER TABLE public.quiz_rewards_daily ENABLE ROW LEVEL SECURITY; REVOKE ALL ON public.quiz_rewards_daily FROM anon, authenticated;
...
-- 확인:
-- SELECT count(*) FROM public.quiz_rewards_daily;                                              -- 0 (아직 없음)
-- SELECT pg_get_functiondef('public.submit_quiz_answer(integer,text,text,boolean,boolean)'::regprocedure);  -- 'capped' 포함
